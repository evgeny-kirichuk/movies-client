...
const selectAllNormalizedMovies = createSelector(
	...
	(_: RootState) => _,
	({ movies }) =>
		movies.reduce((acc: ICategory, value) => {
			if (!acc[value.genre]) {
				acc[value.genre] = [];
			}
...
export const useActiveMovie = () => useSelector(selectActiveMovie);

export const useMoviesLoading = () => useSelector(selectMoviesLoading);

export const useMovieById = (id: string) =>
	useSelector((state: RootState) => selectMovieById(state, id), shallowEqual);

export const useAllMovies = () => useSelector(selectAllMovies, shallowEqual);

export const useAllNormalizedMovies = () =>
	useSelector(selectAllNormalizedMovies, shallowEqual);

export const useRandomMovie = (index: number) =>
	useSelector(
